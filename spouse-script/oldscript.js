...
(async function () {
    ...
    // OVERIDES //
    const $get = (selector) => document.querySelector(selector); const $create = (tag) => document.createElement(tag); const log = console.log;
    const GMSet = async (key, value) => { try { await GM.setValue(key, value); console.log('Value set successfully'); } catch (err) { console.error('Error setting value:', err); } };
    const GMGet = async (key, deflt) => { try { const data = await GM.getValue(key, deflt); if (data !== undefined) { return data; } else { return false; } } catch (err) { console.error('Error setting value:', err); } };
